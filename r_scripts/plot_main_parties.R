...
selected_parties <- c("party.S", "party.F", "party.Vg", "party.Sf")
...
for (party_id in selected_parties) {
  ...
  write.table(party_data, paste0(output_dir, "/", party_id, ".tsv"), sep="\t", row.names = FALSE)
}
...
p <- p +
  ggtitle("SF Proportion for Main Parties (2000 and Up)") +
  xlab("Year") +
  ylab("SF proportion (%)") +
...
ggsave(filename = paste0(output_dir, "/", "main_parties.png"), plot = p, width = 10, height = 6, limitsize = FALSE)
